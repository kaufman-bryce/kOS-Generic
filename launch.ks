...
CLEARSCREEN.
PRINT "Launching to " + ROUND(Ap/1000, 1) + "km orbit".
LOCAL Txt IS "with an inclination of " + round(ABS(Inc), 1).
IF doLAN SET Txt TO Txt + " and a LAN of " + round(ABS(Lan), 1) + " deg.".
ELSE SET Txt TO Txt + " deg.".
PRINT Txt.
...
IF Atmo { //Gravity turn
	...
	WAIT UNTIL VANG(SRFPROGRADE:VECTOR, SHIP:UP:VECTOR) > 10.
}
...
UNTIL ALTITUDE > endAlt AND APOAPSIS >= Ap - ApMgn / 2 {
	...
	SET THRT TO THRT + MAX(-0.01 - ALTPER * 0.04, MIN(dTHRT - THRT, 0.01 + ALTPER * 0.04)). // throttle smoothing
...
WAIT 0.
UNLOCK THROTTLE.
UNTIL SHIP:PARTSTAGGED("launchStage"):EMPTY {STAGE. WAIT 4.}
LOCAL mods IS LIST(
...
UNTIL NOT iter:NEXT{
	...
	FOR p IN SHIP:PARTSTAGGED("launch") {IF partEvent(p, iter:VALUE, ev[iter:INDEX]) SET event TO TRUE.}
	IF event = TRUE {
