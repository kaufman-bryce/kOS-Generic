@LAZYGLOBAL OFF.
//DEPENDS{/lib/node_basic,/lib/node_rdvs}
FUNCTION errDir {
    PARAMETER err.
    IF err < 0 {RETURN -1.}
    else {RETURN 1.}
...
//xfers from parking orbit to moon of current body
FUNCTION nodeMoon {
    PARAMETER moon.
    ...
    until exit {
        ...
        if xfer:ETA > OBT:PERIOD*1.5 {PRINT "Unable to solve transfer.". REMOVE xfer. RUN DIE.}
        makeAp().
