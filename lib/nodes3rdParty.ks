...
function make_node_t_deltav{
    ...
    local rad_unit is -VXCL(pro_unit,body:position-positionat(ship,t)):normalized.
...
function match_orbital_phase{
    ...
    // now dt is time difference [-period/2,period/2] between target's and my periapsis
    local h is 2*obt:semimajoraxis*(1+dt/n/period)^(2/3)-obt:periapsis-2*obt:body:radius.
...
function align_orbital_plane{
    ...
    if VDOT(target_place,vel_at_pe)<0{set true_anomaly to -true_anomaly.}
    local true_anomaly_of_me is VANG(my_place,pe_place).
    if VDOT(my_place,vel_at_pe)<0{set true_anomaly_of_me to -true_anomaly_of_me.}
    local diff is mod(true_anomaly-true_anomaly_of_me+360,360).
    // switch descending to ascending node or vice versa to closer one
    if diff>180{set true_anomaly to true_anomaly-180.}
    ...
    if VDOT(other_normal,vel_at_t)>0{set inc to -inc.}
...
function time_to_pass_height{
    ...
    if e=0{set e to 0.0000001.}
    local c is (h-p)/e/h.
    if c>1{return list(-1).}
    if c<-1{return list(1).}
