...
FUNCTION dateFormat {
	PARAMETER t.
	LOCAL hours IS FLOOR(MOD(t / 60 / 60, 6)).
	LOCAL days  IS FLOOR(MIN(t / 60 / 60 / 6, 9999)).
	IF hours <> 0 AND days <> 0 {
		RETURN days:TOSTRING:PADLEFT(5) + "d" + hours + "h".
	} ELSE RETURN "    -d-h".
}

FUNCTION timeFormat {
	PARAMETER t.
	LOCAL minutes IS FLOOR(MOD(t / 60, 60)).
	LOCAL hours IS FLOOR(MIN(t / 60 / 60, 999)).
	IF hours <> 0 AND minutes <> 0 {
		RETURN hours:TOSTRING:PADLEFT(4) + "h" + minutes:TOSTRING:PADLEFT(2) + "m".
	} ELSE RETURN "   -h -m".
}

FUNCTION clearPrint {
	PARAMETER s, l, x, y.
	PRINT s:TOSTRING:PADRIGHT(l) AT (x,y).
}
...
UNTIL FALSE {
	...
	// Power
	PRINT perBar(res_percent("ElectricCharge"), 14) AT (7, 22).
	PRINT timeFormat(res_ETA("ElectricCharge")) AT (22, 22).
	LOCAL expose IS ROUND(sol:GETFIELD("sun exposure"), 2).
	clearPrint(expose, 4, 13, 20).
	clearPrint(ROUND(MAX(-99.99, MIN(99.99, res_rate("ElectricCharge"))), 2):TOSTRING:PADLEFT(2), 6, 23, 20).
	IF  expose = 1 OR expose = 0 OR panelAim = FALSE {clearPrint(sol:GETFIELD("status"), 18, 11, 21).}
	ELSE {clearPrint("Orienting...", 18, 11, 21).}
	IF panelAim = TRUE {
	...
	FOR e IN docked {
		...
		FOR re IN supplyRes {
			...
			IF xStatus = "IDLE" SET xStatus TO "UNLOADING".
			ELSE SET xStatus TO "BI-DIRECT".
		}
